...
const ExportModal = (() => {
  ...
  const DATASET_CONFIGS = {
    ...
    drugs: {
      ...
      fields: [
        { key: 'name', label: 'Drug Name', selected: true },
        ...
        { key: 'reorder_threshold', label: 'Reorder Threshold', selected: true, getter: d => (d.stock ? d.stock.reorder_threshold : 0) },
        { key: 'stock_status', label: 'Stock Alert', selected: true, getter: d => ((d.stock && d.stock.quantity_on_hand <= d.stock.reorder_threshold) ? 'REORDER ALERT' : 'OK') },
      ],
      filterTypes: ['limit'],
    },
  };
              ...
              <div class="form-grid" style="grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:12px;">
                ...
                <div class="field" id="exp-filter-dept-wrap">
                  ...
                  <select id="exp-filter-dept">
                    ...
                    <option value="Product Quality Control Service">Product Quality Control Service</option>
                    <option value="Production and Technic">Production and Technic</option>
  ...
  function open(defaultDataset = 'patients') {
    ensureModalMarkup();
  ...
  async function performExport() {
    ...
    try {
      ...
      rows.push(['Applied Filters', `Status: ${statusFilter}, Department: ${deptFilter}, Gender: ${genderFilter}, Limit: ${limitFilter}`]);
      rows.push([]);
